...
//                alert('aaaaaa');
    ...
    $(document).on('blur', '.amount', function(){
        var valor = parseFloat($(this).val());
            var total = parseFloat($("#twt").text());
            var pago = parseFloat($("#total_paying").text());
            var pagoA = 0;

            $(".amount").each(function(){
                if($(this).val() > 0){
                    pagoA += parseFloat($(this).val());
                }
                
            });
            if(pagoA >= total){
                var troco = pagoA - total;
                $("#balance").text(troco.toFixed(2));
                $("#balance_val").val(troco.toFixed(2));
                $("#submit-sale").attr('disabled', false);

            }else{
                $("#submit-sale").attr('disabled', true);
                $("#balance").text("0.00");
                $("#balance_val").val("0.00");
            }
            $("#total_paying").text(pagoA.toFixed(2));
    ...
    $(document).on('click', '#submit-sale', function(e){
        ...
        $('#payment-conpag').html('');
        $(".amount").each(function(){
            $('#payment-conpag').append('<input type="hidden" name="amount[]" value="'+parseFloat($(this).val())+'"/>');
        });
    ...
    $(document).on('click', '.adicionarPgto', function(){
        $('#retornaHtml').append('<div class="col-xs-5"><div class="form-group"><select name="paid_by[]" class="form-control paid_by select2" style="width:100%;"><option value="cash">Dinheiro</option><option value="CC">Cartão de Crédito</option> <option value="Stripe">Cartão de Débito</option><option value="Cheque">Cheque</option> </select></div></div><div class="col-xs-6"><div class="form-group"><input name="amount[]" type="text" class="pa form-control kb-pad amount" autofocus/></div></div><div class="col-xs-1"><div class="form-group"></div></div>');
    });
